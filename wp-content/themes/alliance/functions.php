...
function permitir_upload_de_arquivo_stl($mime_types) {
    $mime_types['stl'] = 'application/sla'; // Adicione o tipo MIME correto para arquivos STL
    return $mime_types;
}

add_filter('upload_mimes', 'permitir_upload_de_arquivo_stl');
function adicionar_link_voltar(){
	echo '<div style="text-align: right; margin: -75px 0px 50px 0px">
        <input type="button" value="← Voltar" onclick="voltarPagina()" style="background: transparent; color: #000; padding: 0px;">
    </div>';
}

function adicionar_script_voltar() {
    echo '<script>
...
add_action('wp_footer', 'adicionar_script_voltar');

function add_shortcode_to_product_page() {
    echo '<style>
		.woocommerce .list_products_header .page-title {
			font-size: var(--theme-font-h2_font-size) !important;
			line-height: 1.142em !important;
		}
        .list_products_sorting, .woocommerce ul.products {
			display: none !important;
		}
    </style>';

    echo do_shortcode('[list_products_with_actions]');
}
add_action('woocommerce_before_shop_loop', 'add_shortcode_to_product_page');
...
function redirecionar_usuario_logado() {
    if (is_user_logged_in()) {
        ...
        if ($user->user_login !== 'admin') {
            wp_redirect('https://gomob.online/dentalconnect/');
            exit;
        }
    }
}
add_action('admin_init', 'redirecionar_usuario_logado');

function get_logged_in_full_name() {
    $current_user = wp_get_current_user();
    $first_name = $current_user->first_name;
    $last_name = $current_user->last_name;

    $prefix = "Olá, ";
    $sulfix = ". Bem vindo ao Dental Conect!";

    $full_name = trim("{$prefix} {$first_name} {$last_name}{$sulfix}");
    
    if ($full_name) {
        return '<h2 style="color: #000; margin: 0px;">' . esc_html($full_name) . '</h2>';
    }
}
add_shortcode('user_full_name', 'get_logged_in_full_name');

function customize_admin_menu() {
	$current_user = wp_get_current_user();

    if (in_array('clinica', $current_user->roles) || in_array('dentista', $current_user->roles)) {
		echo '<style>#mobile-menu-item-8992 { display: none !important; }</style>';
    }
}
add_action('wp_head', 'customize_admin_menu');

function get_logged_in_user_role_with_name() {
    $current_user = wp_get_current_user();
    $user_roles = $current_user->roles;
    
    if (!empty($user_roles)) {
        $user_role = $user_roles[0];
        $role_info = get_role($user_role);
        $role_name = $role_info->name;
        
        $output = '<div style="background-color: #0038ae; max-width: 200px; font-size: 12px; margin-bottom:15px; padding: 2px; border-radius: 5px; text-align: center; color: white; text-transform: uppercase;">';
...
add_shortcode('user_role', 'get_logged_in_user_role_with_name');

function show_admin_bar_for_specific_user($show) {
    $current_user = wp_get_current_user();
    return $current_user->ID === 1 ? true : false;
}
add_filter('show_admin_bar', 'show_admin_bar_for_specific_user');

function enqueue_custom_logout_script() {
    wp_enqueue_script('custom_logout_script', get_template_directory_uri() . '/js/custom_logout_script.js', array('jquery'), null, true);
}
