...
if ( ! function_exists( 'trx_addons_sc_igenerator_localize_script' ) ) {
	...
	function trx_addons_sc_igenerator_localize_script( $vars ) {
		...
		$vars['msg_ai_helper_sc_igenerator_limit_exceed'] = trx_addons_get_option( "ai_helper_sc_igenerator_limit_alert_premium" );
		return $vars;
	}
}
	...
	function trx_addons_sc_igenerator_generate_images() {
		...
		if ( ! empty( $prompt ) ) {
			...
			if ( $api->get_api_key() != '' && ! $demo ) {
				...
				if ( ! preg_match( '/^[a-zA-Z0-9 _,:;@#%&\.\-\+\^\(\)\{\}\[\]\\\'\"\*\\\\\/]+$/', $args['prompt'] ) ) {
					$translated = false;
					if ( apply_filters( 'trx_addons_filter_sc_igenerator_auto_translate_prompt', true, $model ) ) {
						...
						if ( $api_translate->get_api_key() != '' ) {
							...
							if ( ! empty( $response['choices'][0]['message']['content'] ) ) {
								$args['prompt'] = $response['choices'][0]['message']['content'];
								$translated = true;
					...
					if ( strpos( $model, 'stabble-diffusion' ) !== false && ! $translated ) {
